...
if(($request["type"] == 0) || isset($inputs->csrf_token) && $inputs->csrf_token == $_SESSION["token"])
{
    ...
    {
        $query = $pdo->prepare("UPDATE request SET in_progress = FALSE, accepted = TRUE WHERE id = :id");
        $query->bindParam(":id", $id);
        $query->execute();
...
<form enctype="multipart/form-data" method="POST" action="/client_decision.php">
    ...
    <div>
        <label for="accepted">Votre client accepte-t-il le bijoux ?</label>
        <input type="checkbox" name="accepted" id="accepted" oninput="on_input()">
    </div>
    ...
    <div id="new_jewel">
        ...
        <div>
            ...
            <input accept="image/jpeg, image/png" type="file" name="image" id="image" onchange="image_on_change()" required>
        </div>
    </div>
